...
const createFakeVolume = async (volumeId: string, imageIds: string[]) => {
  registerFakeLoader();
...
const DicomEditorWithCtImageIds = () => {
  ...
  useEffect(() => {
    const setUp = async () => {
      ...
      const volume = await createFakeVolume(volumeId, imageIds);

      // Set volume vào viewport
      viewport.setVolumes([
        { volumeId: volume.volumeId, callback: setCtTransferFunctionForVolumeActor },
      ]);
...
export default DicomEditorWithCtImageIds;
